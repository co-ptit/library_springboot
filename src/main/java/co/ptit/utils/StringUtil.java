...
import static co.ptit.utils.Constant.HASH_SEED;
...
public class StringUtil {public static final String DOT = ".";
    ...
    public static final Integer PAD_LEFT_DEFAULT = 6;
    ...
    public static boolean isNumeric(String strNum) {
        if (!org.springframework.util.StringUtils.hasLength(strNum)) {
            return false;
        }
    ...
    public static String generateCodeFromId(long id, int padLeft) {
        ...
        return org.apache.commons.lang3.StringUtils.leftPad(code.toString(), padLeft, '0');
    }

    public static String encode(String secret) {
        Assert.isTrue(secret.length() < HASH_SEED.length(), "secret too long " + secret.length());
        StringBuilder encode = new StringBuilder();
        for (int i = 0; i < secret.length(); i++) {
            encode.append((secret.charAt(i) ^ HASH_SEED.charAt(i)));
    ...
    public static String decode(String secret) {
        String[] en = secret.split(",");
        Assert.isTrue(en.length < HASH_SEED.length(), "secret too long " + en.length);
        StringBuilder decode = new StringBuilder();
        for (int i = 0; i < en.length; i++) {
            int out = (Integer.parseInt(en[i]) ^ HASH_SEED.charAt(i));
            decode.append((char) out);
        }
